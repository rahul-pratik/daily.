...
export const OnboardingModal: React.FC<OnboardingModalProps> = ({
  ...
  // Google Sign-In Handler: Opens redesigned Google Account Chooser screen
  ...
  // Select and Authenticate with Google Account (1-click direct login, no password, no confirm email)
  const handleSelectGoogleAccount = async (selectedEmail: string, selectedName: string) => {
    ...
    try {
      ...
      const cleanName = selectedName.trim() || cleanEmail.split('@')[0];
      const cleanUsername = cleanEmail.split('@')[0].toLowerCase().replace(/[^a-z0-9_]/g, '');

      // Execute Supabase Google Auth & instant database sync
      const res = await supabaseSignInWithGoogle(cleanEmail, cleanName, avatar);
      ...
      // Finalize and enter Daily with Google profile
      handleAuthComplete('google', cleanEmail, {
        id: res.user?.id,
        name: cleanName,
        username: cleanUsername,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/90 backdrop-blur-md animate-in fade-in duration-200">
      <div className="w-full max-w-md bg-[#0A0A0A] border border-white/10 rounded-[32px] p-5 sm:p-6 shadow-2xl relative text-white flex flex-col max-h-[92vh] overflow-hidden">
        ...
        {/* =================================================================== */}
        {/* REDESIGNED GOOGLE SIGN-IN & ACCOUNT CHOOSER SCREEN                  */}
        {/* =================================================================== */}
        {showGoogleChooser && (
          <div className="absolute inset-0 z-50 bg-[#0C0E12] rounded-[32px] p-6 flex flex-col justify-between animate-in fade-in zoom-in-95 duration-200 border border-white/20 shadow-2xl">
            <div className="space-y-4 overflow-y-auto pr-1">
              ...
              <div className="flex flex-col items-center text-center pt-2 pb-1">
                <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center shadow-lg shadow-white/10 mb-3 shrink-0">
                ...
                <p className="text-xs text-white/60 mt-1">Choose an account to continue to Daily</p>
              </div>
              ...
              {/* Primary Google Account Card - 1-Click Login, saves to Supabase */}
              <div className="space-y-2">
                <button
                  type="button"
                  onClick={() =>
                    handleSelectGoogleAccount(
                      email.trim().includes('@gmail.com') ? email.trim() : 'pratik.rahulb@gmail.com',
                      name.trim() || 'Rahul Pratik'
                    )
                  }
                  disabled={authLoading === 'google'}
                  className="w-full p-3.5 rounded-2xl bg-white/10 hover:bg-white/15 active:scale-[0.99] border border-white/15 hover:border-blue-500/50 transition-all flex items-center justify-between group cursor-pointer text-left"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <span className="text-[11px] text-white/70 block truncate font-mono">
                        {email.trim().includes('@gmail.com') ? email.trim() : 'pratik.rahulb@gmail.com'}
                      </span>
                    </div>
                  </div>
                ...
                {!showCustomGoogleInput ? (
                  <button
                    ...
                    className="w-full p-3 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/10 transition-colors text-left flex items-center gap-3 cursor-pointer"
                  >
                    <div className="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center text-white/60 shrink-0">
                      <UserIcon className="w-4 h-4" />
                    </div>
                    <span className="text-xs font-semibold text-white/80">Use another Google account</span>
                  </button>
                ) : (
                  <div className="p-3 rounded-2xl bg-white/5 border border-white/10 space-y-2 animate-in fade-in duration-150">
                    ...
                    <div className="relative flex items-center">
                      ...
                      <input
                        ...
                        className="w-full pl-9 pr-3 py-2 bg-black/40 border border-white/15 focus:border-[#2F6FED] rounded-xl text-xs text-white placeholder-white/30 outline-none"
                      />
                    </div>
                    <button
                      type="button"
                      onClick={() => {
                        if (customGoogleEmail.trim().includes('@')) {
                          ...
                          setAuthError('Please enter a valid Google email.');
                        }
                      }}
                      disabled={!customGoogleEmail.trim() || authLoading === 'google'}
                      className="w-full py-2 rounded-xl bg-[#2F6FED] hover:bg-blue-600 text-white font-bold text-xs uppercase tracking-wider cursor-pointer transition-colors"
                    >
              ...
              <p className="text-[10px] text-white/40 text-center leading-relaxed px-2 pt-2">
                To continue, Google will share your name, email address, and profile picture with Daily.
              </p>
            </div>
            ...
            <div className="pt-3 border-t border-white/10 shrink-0">
              <button
                ...
                className="w-full py-2.5 rounded-2xl bg-white/5 hover:bg-white/10 text-white/70 hover:text-white font-bold text-xs transition-colors cursor-pointer"
              >
                Back to Sign Up
              </button>
            </div>
